...
names <- unique(samples_sle$orig.sample)

disease <- grep("SLE", names, value = TRUE)
samples_sle$condition[samples_sle$orig.sample %in% disease] <- "Disease"


healthy <- grep("HD", names, value = TRUE)
samples_sle$condition[samples_sle$orig.sample %in% healthy] <- "Healthy"
...
# Group By Cell Type
DimPlot(samples_mouse, reduction="umap.integrated", label = TRUE, group.by="clusters.integrated", raster=FALSE)

DimPlot(samples_mouse, reduction="umap.integrated", label = TRUE, group.by="SingleR_main.sct", raster=FALSE) + NoLegend()

DimPlot(samples_mouse, reduction="umap.integrated", label = TRUE, label.size=6, group.by="SingleR_fine.sct", raster=FALSE) + NoLegend()
...
# Cell Type
samples_sle$cell_type <- "Other" # samples_sle@active.ident

samples_sle$cell_type[samples_sle@active.ident %in% c(17)] <- "Treg"
...
deseq2_results <- runDESeq2(treg_sle, includeCellType = FALSE)

res <- as.data.frame(deseq2_results$results)
res <- res[!is.na(res$log2FoldChange), ]
res$avg_log2FC <- res$log2FoldChange
res$gene <- rownames(res)

gsea_results_msigdbr <- runGSEA(res, database = "METABOLIC", species = "human")
...
# For All Other Pathways
ridgeplot(gsea_results_msigdbr) +
  ggtitle(paste(type, "GSEA Ridge Plot"))
dotplot(gsea_results_msigdbr, showCategory = 20) +
  ggtitle(paste(type, "GSEA Dot Plot"))
enrichplot::gseaplot2(gsea_results_msigdbr) +
  ggtitle(paste(type, "GSEA Enrichment Plot"))


# Only for METABOLIC
print(gsea_results_msigdbr$bar_plot)
print(gsea_results_msigdbr$dot_plot)
print(gsea_results_msigdbr$enrichment_plot)
